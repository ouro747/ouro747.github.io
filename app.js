...
function productCard(p){const img=p.media?.find(m=>m.src)?.src;const presentation=p.presentationFromUser||[p.concentration,p.format].filter(has).join(' • ')||'Apresentação conforme rótulo';return `<article class="product-card"><a href="/produto/${esc(p.slug)}" data-route class="product-media">${img?`<img src="${esc(img)}" alt="${esc(p.displayName)}">`:`<div class="product-placeholder">▧</div>`}${docPill(p)}</a><div class="product-body"><h3><a href="/produto/${esc(p.slug)}" data-route>${esc(p.name)}</a></h3><div class="product-meta">${esc(presentation)}</div><div class="product-price-row"><div><div class="price-kicker">${esc(c.fromPriceLabel||'A partir de')}</div><div class="price">${money(p.promoPriceCents?p.promoPriceCents/100:p.pricing.basic)}</div>${p.expressAvailable?`<div class="express-price">Expressa ${money(p.pricing.express)}</div>`:''}</div><button class="cart-mini" data-add="${esc(p.slug)}" aria-label="Adicionar ao carrinho">🛒</button></div><div class="product-cta"><a class="btn btn-primary" href="/produto/${esc(p.slug)}" data-route>VER PRODUTO</a><button class="cart-mini" data-add="${esc(p.slug)}">🛒</button></div></div></article>`}
